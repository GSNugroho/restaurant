    ...
    function get_data_stok(){
        $.get("<?php echo base_url().'Bersih/get_stok_all_bersih'?>", function(data){
            ...
            for(var i=0; i < data.produk.length; i++){
                // if(data.produk[i].produk_kategori != 'HT-000005'){
                    var masuk = 0;
                    var keluar = 0;
                    var stok_awal = 0;
                    var stok_masuk = 0;
                ...
                //         }
                //     }
                //     html2 += `<td>${(stok_masuk).toFixed(2)}</td>`;
                ...
                //         }
                //     }
                //     html2 += `<td>${(stok_keluar).toFixed(2)}</td>`;
                    
                //     var akhir = (parseFloat(masuk - keluar)).toFixed(2);
                //     switch(kd_produk){
                //         // // sambal masak
                //         // case 'RO-000009': 
                //         //     jml_per_porsi = 0.05;
                ...
                //     html2 += `<td>${(parseFloat(masuk - keluar)).toFixed(2)}</td><td>${data.produk[i].produk_satuan}</td><td>${Math.floor(akhir/jml_per_porsi)}</td><td class="text-right">Rp ${uang(Math.floor(((parseFloat(masuk - keluar)).toFixed(2)) * harga))}</td></tr>`;
                //     total_aset2 += Math.floor(((parseFloat(masuk - keluar)).toFixed(2)) * harga)
                // }
